...
> **현재 상태:** 계정 활성화·로그인, 공개 제안·동의, 신고·심의, 익명 Identity Vault와 승인된 신원 열람 경로는 구현되어 있다. 비공개 고충과 재정 공개는 아직 배포 범위가 아니며, 이 저장소는 운영 승인을 받지 않았다. 아래에 남아 있는 과거의 "미구현" 표현보다 `docs/requirements-checklist.md`를 구현 상태의 기준으로 사용한다.

`docker-compose.yml`은 로컬 개발 전용이다. 운영 후보 산출물은 `backend/Dockerfile`, `frontend/Dockerfile`, `deploy/docker-compose.prod.yml`이며, 실제 이미지 digest, TLS, 비밀 저장소, 백업, 보존 정책, 운영자와 CODEOWNERS가 확정되기 전에는 운영에 사용하지 않는다.
