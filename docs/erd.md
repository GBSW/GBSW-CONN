...
| `identity_reveal_records` | 실제 신원 열람 감사 | 승인 사건, 고정 심의자, 열람 기간, 재인증을 서비스에서 검증 |
...
의결은 사건 행을 먼저 잠그고 심의자 스냅샷당 고유 제약으로 1표만 허용한다. 반대가 있으면 반려하고 세 승인표가 모두 저장된 경우에만 승인한다. 공개 제한 사건 승인 시 공개 상태와 이력을 같은 트랜잭션으로 기록한다. 신원 확인은 승인 사건 행을 잠근 뒤 열람 행을 추가한다. 사건당 한 건이라는 고유 제약은 세 심의자의 반복 열람을 위해 제거했고, 열람 가능 기간은 사건의 확정 시각을 기준으로 서비스가 판정한다.
